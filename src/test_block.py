...
from block_text import split_block, block_to_block_type, markdown_to_html_node
...
class TestSplitBlock(unittest.TestCase):
    ...
    def test_split_blocks_no_splits(self):
        text = ""
        split_blocks = split_block(text)
        self.assertListEqual(split_blocks, [""])
...
class TestMarkdownToHTMLNode(unittest.TestCase):
    ...
    def test_h1(self):
        ...
        comp = ParentNode(
            ...
            children=[
                ParentNode(
                    "h1", children=[LeafNode(tag=None, value="This is a heading 1")]
                )
            ],
        )
    ...
    def test_h2(self):
        ...
        comp = ParentNode(
            ...
            children=[
                ParentNode(
                    "h2", children=[LeafNode(tag=None, value="This is a heading 2")]
                )
            ],
        )
    ...
    def test_blockquote(self):
        blockquote = markdown_to_html_node(self.blockquote)
        comp = ParentNode(
            "div",
            children=[
                ParentNode(
                    ...
                    children=[
                        LeafNode(
                            tag=None, value="This is part of a quote that has an "
                        ),
                        LeafNode(tag="i", value="italic"),
                        LeafNode(tag=None, value=" element"),
                    ],
                )
            ],
        )
        self.assertEqual(comp, blockquote)
